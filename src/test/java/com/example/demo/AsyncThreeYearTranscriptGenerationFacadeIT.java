...
import com.example.demo.service.event.ThreeYearTranscriptGenerationRequestedService;
import java.io.File;
...
class AsyncThreeYearTranscriptGenerationFacadeIT extends FacadeIT {
  ...
  @Autowired
  private ThreeYearTranscriptGenerationRequestedService
      threeYearTranscriptGenerationRequestedService;
  ...
  @Test
  void requestingAFullTranscriptProducesAnEventThatTheWorkerTurnsIntoAThreeYearPdfAndAnEmail() {
    var teacher = saveUser("i7.teacher@hei.school", Role.TEACHER, null);
    var student = saveUser("i7.student@hei.school", Role.STUDENT, "STD970001");

    var promotion = new Promotion();
    promotion.setLabel("Promo I7");
    promotion.setExpectedGraduationYear(2053);
    var savedPromotion = promotionRepository.save(promotion);

    var year1 = saveAcademicYear(LocalDate.of(2050, 9, 1), LocalDate.of(2051, 7, 1));
    var year2 = saveAcademicYear(LocalDate.of(2051, 9, 1), LocalDate.of(2052, 7, 1));
    var year3 = saveAcademicYear(LocalDate.of(2052, 9, 1), LocalDate.of(2053, 7, 1));

    enrollStudentWithGrade(student, teacher, savedPromotion, year1, "I7-C1", 12.0);
    enrollStudentWithGrade(student, teacher, savedPromotion, year2, "I7-C2", 14.0);
    enrollStudentWithGrade(student, teacher, savedPromotion, year3, "I7-C3", 16.0);
    ...
    var response =
        restTemplate.exchange(
            url("/students/" + student.getId() + "/transcripts/full"),
            HttpMethod.POST,
    ...
    verify(threeYearEventProducer).accept(eventCaptor.capture());
    var producedEvents = eventCaptor.getValue();
    assertThat(producedEvents).hasSize(1);
    var producedEvent = (ThreeYearTranscriptGenerationRequested) producedEvents.get(0);
    assertThat(producedEvent.getStudentId()).isEqualTo(student.getId());

    threeYearTranscriptGenerationRequestedService.accept(producedEvent);
    ...
    assertThat(bucketKeyCaptor.getValue())
        .isEqualTo("transcripts/" + student.getId() + "/full-path.pdf");
    ...
    assertThat(emailCaptor.getValue().to().getAddress()).isEqualTo("i7.student@hei.school");
    assertThat(emailCaptor.getValue().attachments()).containsExactly(fileCaptor.getValue());
  }

  private void enrollStudentWithGrade(
      User student,
      User teacher,
      Promotion promotion,
    ...
    var course = saveCourse(courseRef, courseRef + " course", 30);
